...
// --- API: Universal Direct Download (MP4 & HLS via FFmpeg) ---
app.get('/api/download/direct', async (req, res) => {
    const videoUrl = req.query.url;
    const title = req.query.title || 'X-Stream_Video';
    ...
    const isHls = videoUrl.includes('.m3u8') || videoUrl.includes('master.') || videoUrl.includes('media=hls') || videoUrl.includes('teenxy.com/get_file/');

    if (isHls && ffmpegAvailable) {
        console.log(`📥 [FFmpeg Download] Starting for: ${safeTitle}`);

        let headersArr = [`User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36`];
        if (referer) {
            headersArr.push(`Referer: ${referer}`);
            try { headersArr.push(`Origin: ${new URL(referer).origin}`); } catch (e) { }
        }
        const headersStr = headersArr.join('\r\n') + '\r\n';
        ...
        const command = ffmpeg(videoUrl)
            ...
            .on('end', () => console.log(`✅ [FFmpeg Download] Finished for ${safeTitle}`));
        ...
        // Direct stream for MP4
        console.log(`📥 [Direct Download] Streaming MP4 for: ${safeTitle}`);
        try {
            const response = await axios({
                method: 'get',
                url: videoUrl,
