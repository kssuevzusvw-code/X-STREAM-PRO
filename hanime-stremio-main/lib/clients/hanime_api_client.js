...
class HanimeApiClient {
  constructor(config) {
    ...
    // Global axios defaults for this client
  ...
  async search({ query = '', tags = [], orderBy = 'created_at_unix', ordering = 'desc', page = 0 }, maxRetries = 2) {
    ...
    try {
      return await this._retryRequest(
        async () => {
          const response = await this.axios.post(this.searchUrl, body, {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (status === 403 && attempt < maxRetries) {
          const backoffDelay = [500, 1000, 2000][attempt] || 2000; // Reduced from 2s, 5s, 10s to 500ms, 1s, 2s
